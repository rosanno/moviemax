          ...
          {nowPlaying?.map((movie: MovieType) => (
            ...
            >
              ...
              <div className="w-full md:max-w-6xl md:px-10 2xl:max-w-[1800px] mx-auto">
                <div className="absolute bottom-10 md:bottom-52 pl-3 md:pl-0 pr-4 w-sm md:w-xl 2xl:w-2xl">
                  <h1 className="text-4xl md:text-5xl 2xl:text-6xl font-bold text-white mb-1">
                    {movie.title}
                  </h1>
                  <p className="text-white text-sm md:text-base mb-2 md:mb-3 italic">
                    {isLoading
                      ? "Loading genres..."
                      : movie.genre_ids
                          .map((id) => genres?.[id])
